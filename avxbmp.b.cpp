...
void profile_avx512(size_t size, size_t iter_count = 200) {
    ...
    for (size_t i = 0; i<iter_count; i++) {
        avxbmp::gen_bitmap_avx512(bitmap_data.data(), &null_count, nulldata.data(), nulldata.size());
    }
...
//     //  populating nulldata by 0 1 0 1 ...
//     for (size_t i=0; i<nulldata.size(); i++) {
...
//     auto start = std::chrono::high_resolution_clock::now();
//     for (int i = 0; i<iter_count; i++) {
//         gen_bitmap_avx512(bitmap_data.data(), &null_count, nulldata.data(), nulldata.size());
//     }
//     size_t dur = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now()-start).count(); 
//     double dur_per_iter = dur/iter_count;
//     double throughput_gibs = size*sizeof(TYPE)*1.0e9/dur_per_iter/(1024*1024*1024);
