...
static void test_bitmap_creator(
        const size_t size, 
        ...
        bool verbose = false) 
{
    ...
    if (verbose) {
        std::cout 
            << avxbmp::helpers::get_type_name<TYPE>() 
            <<"; Expected bitmap: "; 
    ...
    size_t diff_count = avxbmp::diffBitmap(actual_bitmap_data, expected_bitmap_data, verbose);
...
static void testSimpleAVX512(const size_t size, bool verbose = false) {
    ...
    test_bitmap_creator<TYPE>(size, std::move(bitmap_creator), std::move(nulldata_populator), verbose);
}
