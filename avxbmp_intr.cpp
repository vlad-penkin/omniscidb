...
extern "C" __attribute__((target("avx512bw"), optimize("no-tree-vectorize"))) size_t
gen_bitmap_avx512_8_intr(uint8_t* dst,
  ...
  // DOES NOT COMPILE: __m512i nulls_mask =
  // _mm512_broadcastq_epi64(reinterpret_cast<__m128i>(_mm_load_pd(reinterpret_cast<double*>(&null_val)));
...
gen_bitmap_avx512_32_intr(uint8_t* dst,
  ...
  while (size > 0) {
    ...
    *reinterpret_cast<__mmask16*>(dst) = k1;  // mov         %ax, 0(%rdi) : TODO CHANGE ACCORDINGLY
    ...
    // std::cout << "#k1: " << std::bitset<16>(*reinterpret_cast<int16_t*>(&k1)) << std::endl;
    // # counting nulls, storing nulls count to %rax
