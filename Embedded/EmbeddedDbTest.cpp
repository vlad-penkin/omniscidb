...
// Input files' names
constexpr char * TABLE6x4_CSV_FILE = "../../Tests/EmbeddedDataFiles/embedded_db_test_6x4table.csv";
constexpr char * JOIN_TABLE_CSV_FILE = "../../Tests/EmbeddedDataFiles/embedded_db_test_join_table.csv";
constexpr char * NULLSTABLE6x4_CSV_FILE = "../../Tests/EmbeddedDataFiles/embedded_db_test_nulls_table.csv";
...
namespace {
  ...
  std::string  parse_cli_args(int argc, char* argv[]) {
    ...
    desc.add_options()
      ...
      ("data,d", po::value<std::string>(&catalog_path)->default_value(catalog_path), "Directory path to OmniSci catalogs")
      ("calcite-port,p", po::value<int>(&calcite_port)->default_value(calcite_port), "Calcite port")
      ("enable-columnar-output,o", po::value<bool>(&columnar_output)->default_value(columnar_output),"Enable columnar_output")
      ;
  ...
  void load_table6x4_csv (std::string csv_file, std::string table_name, size_t fragment_size) {
    if (!g_dbe) {
...
//  NOTE: load_table6x4_csv() creates table with Arrow types <dictionary, int32, int64, float64>
//  so we check that.
...
void test_chunked_conversion(bool enable_columnar_output, bool enable_lazy_fetch, size_t fragment_size) {
...
void JoinTest(bool enable_columnar_output, bool enable_lazy_fetch) {
  ...
  // compare_columns(std::array<bool,6>{false, false, false, true, true, true}, table->column(5));  
...
TEST(DBEngine, ArrowTableChunked_NULLS1) {
  ...
  compare_columns(std::array<int32_t,6>{i32_null, 0, i32_null, i32_null, 1, 1}, table->column(1));
  compare_columns(std::array<int64_t,6>{i64_null, 2, 3, 4, i64_null, 6}, table->column(2));
  compare_columns(std::array<double,6>{10.1, f64_null, f64_null, 40.4, 50.5, f64_null}, table->column(3));
}
