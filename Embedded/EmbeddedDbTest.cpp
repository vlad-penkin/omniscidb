...
template<> struct ConversionTraits<uint32_t> {
                 ...
                 std::string table_name = "table1")
{
  ...
  size_t fragment_size = fragments_count > 1 ? size_t((values.size()+fragments_count-1)/fragments_count) : 0;
...

  {
    ...
    auto schema = arrow::schema({arrow::field("value", ConversionTraits<TYPE>::getSchemaValue())});
...
void test_single_column_table (size_t N, size_t fragments_count) {
  std::vector<TYPE> int32_values(N,0);

  for (size_t i = 0; i < int32_values.size(); i++) {
    int32_values[i] = (i % 2) == 0 ? 0 : null_builder<TYPE>();
  }

  build_table<TYPE>(int32_values, fragments_count, "LargeTable");
  ...
  ASSERT_EQ(table->num_rows(), (int64_t)int32_values.size());
  compare_columns(int32_values, table->column(0));  
...
TEST(DBEngine, LargeTables) {
  ...
  // test_single_column_table<uint32_t>(30'000'000, 150);  //C++ exception with description "uint32 is not yet supported." thrown in the test body.
  test_single_column_table<int64_t>(30'000'000, 150);
  // test_single_column_table<uint64_t>(30'000'000, 150);     // C++ exception with description "uint64 is not yet supported." thrown in the test body.
