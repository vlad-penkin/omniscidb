...
void ExecutionKernel::run(Executor* executor, SharedKernelContext& shared_context) {
  DEBUG_TIMER_NAME("ExecutionKernel::run");
...
void ExecutionKernel::runImpl(Executor* executor, SharedKernelContext& shared_context) {
...
#ifdef DBG_PRINT
  {
    ...
    std::cout << "Has lazy fetch columns: "
              << executor->hasLazyFetchColumns(ra_exe_unit_.target_exprs) << std::endl;
  }
#endif
  can_run_subkernels =
  ...
  // structure to hold all ResultSets and hold chunks is
  // required.
#ifdef DBG_PRINT
  {
    std::lock_guard<std::mutex> loc(dbg_print_mutex);
    std::cout << "Need to hold chunks: " << need_to_hold_chunk(chunks, ra_exe_unit_)
              << std::endl;
  }
#endif
  ...
  if (can_run_subkernels) {
    ...
    if (query_mem_desc.cpuThreadsShareMemory()) {
      std::lock_guard<std::mutex> lock(shared_context.shared_execution_context_mutex);
      if (!shared_context.shared_execution_context) {
        shared_context.shared_execution_context =
    ...
    for (size_t sub_start = start_rowid; sub_start < total_rows; sub_start += sub_size) {
      ...
      {
        std::lock_guard<std::mutex> loc(dbg_print_mutex);
        std::cout << "Create a subtask for [" << sub_start << ", "
                  << (sub_start + sub_size) << ") range, input is "
                  << total_num_input_rows << " rows" << std::endl;
      }
#endif
      auto subtask = std::make_unique<KernelSubtask>(
          *this, shared_context, fetch_result, total_num_input_rows, sub_start, sub_size);
      shared_context.thread_pool->spawn(
          [subtask{std::move(subtask)}, executor] { subtask->run(executor); });
    }
...
void KernelSubtask::run(Executor* executor) {
  QueryExecutionContext* query_exe_context =
      shared_context_.shared_execution_context.get();
  ...
  {
    std::lock_guard<std::mutex> loc(dbg_print_mutex);
    std::cout << "Running a subtask in thread #" << tbb::this_tbb_thread::get_id()
              << ": range=[" << start_rowid_ << ", " << (start_rowid_ + num_rows_) << ")"
              << std::endl;
  }
#endif
  ...
  if (!query_exe_context) {
    ...
    if (!query_exe_context_owned) {
      try {
        // We pass fake col_buffers and frag_offsets. These shouldn't be used
        // when subtasks are used. Don't pass empty structures to avoid empty
        // results.
#ifdef DBG_PRINT
        {
          std::lock_guard<std::mutex> loc(dbg_print_mutex);
          std::cout << "Initializing execution context in thread #"
                    << tbb::this_tbb_thread::get_id() << std::endl;
        }
#endif
  ...
  /*
    {
      std::lock_guard<std::mutex> loc(dbg_print_mutex);
      std::cout << "Got subtask results for [" << start_rowid_ << ", "
                << (start_rowid_ + num_rows_) << ") range with " << results->rowCount()
                << " row(s)" << std::endl;
    }
    shared_context_.addDeviceResults(std::move(results), outer_tab_frag_ids);
    query_exe_context_owned.reset();
  */
}
