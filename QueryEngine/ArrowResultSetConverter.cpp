...
static std::pair<size_t, size_t> compute_adjusted_sizes(size_t size) {
  CHECK(size < std::numeric_limits<size_t>::max() / sizeof(TYPE));
  size_t size_bytes = size * sizeof(TYPE);
  size_t rem_size_bytes = size_bytes & 0b00111111;
  size_t rounded_size_bytes = size_bytes - rem_size_bytes;
...
size_t gen_bitmap(uint8_t* bitmap, const TYPE* data, size_t size) {
  ...
  if constexpr (sizeof(TYPE) == 1) {
    ...
    return gen_null_bitmap_32(bitmap,
                              reinterpret_cast<const uint32_t*>(data),
                              size,
                              reinterpret_cast<const uint32_t&>(nullval));
  } else if constexpr (sizeof(TYPE) == 8) {
    return gen_null_bitmap_64(bitmap,
                              reinterpret_cast<const uint64_t*>(data),
                              size,
                              reinterpret_cast<const uint64_t&>(nullval));
  } else {
    throw std::runtime_error(
        "gen_bitmap() -- Unsupported type: " + typeName((TYPE*)nullptr) + ". Aborting.");
  }
}
std::mutex mtx;

template <typename TYPE>
void create_bitmap_parallel_for_avx512(uint8_t* bitmap_data,
  ...
  auto [avx512_processing_count, cpu_processing_count] =
      compute_adjusted_sizes<TYPE>(vals_size);

  const size_t min_block_size = 64 / sizeof(TYPE);

  const size_t thread_count = std::thread::hardware_concurrency();
  const size_t min_blocks_per_thread_count =
      std::max<size_t>(1, avx512_processing_count / thread_count / min_block_size);
  const size_t block_size = min_blocks_per_thread_count * min_block_size;

  size_t processor_block_size = block_size;

  auto br_par_processor = [&](const tbb::blocked_range<size_t>& r) {
    size_t idx = processor_block_size * r.begin();
    size_t processing_count =
        std::min(processor_block_size * r.end() - idx, avx512_processing_count - idx);
  ...
  threading::parallel_for(
      tbb::blocked_range<size_t>(
          0, (avx512_processing_count + processor_block_size - 1) / processor_block_size),
      br_par_processor);
...
void convert_column(ResultSetPtr result,
  ...
  threading::parallel_for(static_cast<size_t>(0), values.size(), [&](size_t idx) {
    ...
    create_bitmap_parallel_for_avx512<null_type_t<C_TYPE>>(
        bitmap, null_count, vals, chunk_rows_count);
