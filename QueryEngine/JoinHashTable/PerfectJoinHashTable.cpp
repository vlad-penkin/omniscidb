...
void PerfectJoinHashTable::reify() {
  ...
  const auto cols = get_cols(qual_bin_oper_.get(),
                             executor_->getSchemaProvider(),
...
HashJoinMatchingSet PerfectJoinHashTable::codegenMatchingSet(const CompilationOptions& co,
  ...
  auto hash_join_idx_args = getHashJoinArgs(pos_ptr, key_lv, key_col, co);
...
llvm::Value* PerfectJoinHashTable::codegenSlot(const CompilationOptions& co,
  ...
  const auto cols = cols_and_string_op_infos(
  ...
  CHECK(hash_ptr);
  const auto hash_join_idx_args = getHashJoinArgs(hash_ptr, key_lv, key_col, co);
  const auto& key_col_ti = key_col->get_type_info();
