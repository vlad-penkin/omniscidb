...
static inline ALWAYS_INLINE DEVICE int64_t
fixed_width_int_decode_impl(const int8_t* byte_stream,
...
SUFFIX(fixed_width_int_decode)(const int8_t* byte_stream,
                               const int32_t byte_width,
                               const int64_t pos) {
  return fixed_width_int_decode_impl(byte_stream, byte_width, pos);
}

static inline ALWAYS_INLINE DEVICE int64_t
fixed_width_unsigned_decode_impl(const int8_t* byte_stream,
                                 const int32_t byte_width,
                                 const int64_t pos) {
#ifdef WITH_DECODERS_BOUNDS_CHECKING
  assert(pos >= 0);
...
SUFFIX(fixed_width_unsigned_decode)(const int8_t* byte_stream,
                                    const int32_t byte_width,
                                    const int64_t pos) {
  return fixed_width_unsigned_decode_impl(byte_stream, byte_width, pos);
}
...
SUFFIX(diff_fixed_width_int_decode)(const int8_t* byte_stream,
                                    ...
                                    const int64_t pos) {
  return fixed_width_int_decode_impl(byte_stream, byte_width, pos) + baseline;
}

static inline ALWAYS_INLINE DEVICE float fixed_width_float_decode_impl(
...
extern "C" DEVICE ALWAYS_INLINE float SUFFIX(
  ...
  return fixed_width_float_decode_impl(byte_stream, pos);
}
...
static inline ALWAYS_INLINE DEVICE double fixed_width_double_decode_impl(
    const int8_t* byte_stream,
    const int64_t pos) {
#ifdef WITH_DECODERS_BOUNDS_CHECKING
  assert(pos >= 0);
...
extern "C" DEVICE ALWAYS_INLINE double SUFFIX(
  ...
  return fixed_width_double_decode_impl(byte_stream, pos);
}
...
static inline ALWAYS_INLINE DEVICE int64_t
fixed_width_small_date_decode_impl(const int8_t* byte_stream,
                                   const int32_t byte_width,
                                   const int32_t null_val,
                                   const int64_t ret_null_val,
                                   const int64_t pos) {
  auto val = fixed_width_int_decode_impl(byte_stream, byte_width, pos);
...
SUFFIX(fixed_width_small_date_decode)(const int8_t* byte_stream,
                                      ...
                                      const int64_t ret_null_val,
                                      const int64_t pos) {
  return fixed_width_small_date_decode_impl(
