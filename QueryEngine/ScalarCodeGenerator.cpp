...
ScalarCodeGenerator::CompiledExpression ScalarCodeGenerator::compile(
  ...
  unsigned int AS = (co.device_type == ExecutorDeviceType::L0) ? 1 : 0;
  ...
  arg_types[0] =
      llvm::PointerType::get(llvm_type_from_sql(expr->get_type_info(), ctx), AS);
  ...
  if (co.device_type == ExecutorDeviceType::GPU ||
    ...
    wrapper_arg_types[0] = llvm::PointerType::get(get_int_type(32, ctx), AS);
    wrapper_arg_types[1] = arg_types[0];
    for (size_t i = 1; i < arg_types.size(); ++i) {
      wrapper_arg_types[i + 1] = llvm::PointerType::get(arg_types[i], AS);
    }
    auto wrapper_ft =
